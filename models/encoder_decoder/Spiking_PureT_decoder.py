...
class DecoderLayer(nn.Module):
    def __init__(self, embed_dim=512, num_heads=8, dropout=0.1, ff_dropout=0.1, use_gx=False):
        ...
        self.cross_att = SMultiHeadSelfAttention(
            embed_dim = embed_dim,
            num_heads = num_heads
        )
...
class SMultiHeadSelfAttention(nn.Module):
    def __init__(self, embed_dim=512, num_heads=8):
        super().__init__()
        self.embed_dim = embed_dim
        self.num_heads = num_heads
    ...
    def forward(self, q, k, v, mask):
        ...
        q = self.q_lif(q).view(B_, -1, self.num_heads, self.head_dim).transpose(1, 2).contiguous()
