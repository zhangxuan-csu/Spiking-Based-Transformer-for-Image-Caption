...
class EncoderLayer(nn.Module):
    def __init__(
            ...
            mlp_ratio=4,       # SpikingFeedForward 中间层维度变换
            dropout=0.1,
            use_gx=False
    ):
        ...
        # 构造SpikingFeedForward层
        ffn_embed_dim = int(embed_dim * mlp_ratio)
        self.ff_layer = SpikingFeedForward(
            embed_dim = embed_dim,
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, embed_dim=512, window_size=(12, 12), num_heads=8,
        ...
        self.o_linear = nn.Linear(embed_dim, embed_dim)
        self.o_bn = nn.BatchNorm1d(embed_dim)
        self.o_lif = PLMP()
    ...
    def forward(self, x, mask=None):
        ...
        q = self.q_lif(q).view(B_, -1, self.num_heads, self.head_dim).transpose(1, 2)

        k = self.k_linear(x)
        k = self.k_bn(k.transpose(-1, -2)).transpose(-1, -2)
        k = self.k_lif(k).view(B_, -1, self.num_heads, self.head_dim).transpose(1, 2).contiguous()

        v = self.v_linear(x)
        v = self.v_bn(v.transpose(-1, -2)).transpose(-1, -2).view(B_, -1, self.num_heads, self.head_dim).transpose(1, 2)
        v = self.v_lif(v).view(B_, -1, self.num_heads, self.head_dim).transpose(1, 2).contiguous()
        ...
        # x = self.o_linear(x)
        x = self.o_linear(x)
        x_t = self.o_bn(x.transpose(-1, -2)).transpose(-1, -2)
        x = self.o_lif(x_t)
        return x
