...
class PLMP(nn.Module):
    ...
    """
    def __init__(self):
        super(PLMP, self).__init__()
        init_tau = - math.log(tau - 1)
        self.tau1 = nn.Parameter(torch.tensor(init_tau, dtype=torch.float))
        self.tau2 = nn.Parameter(torch.tensor(init_tau, dtype=torch.float))
        self.tau3 = nn.Parameter(torch.tensor(init_tau, dtype=torch.float))
        init_vth1 = Vth
        init_vth2 = Vth2
        init_vth3 = Vth3
        self.vth1 = nn.Parameter(torch.tensor(init_vth1, dtype=torch.float))
        self.vth2 = nn.Parameter(torch.tensor(init_vth2, dtype=torch.float))
        self.vth3 = nn.Parameter(torch.tensor(init_vth3, dtype=torch.float))
    ...
    def forward(self, x):
        ...
        for t in range(TimeStep):
            u = self.tau1.sigmoid() * u * (1 - spikefunc(u)) + x[t * bs:(t + 1) * bs, ...]
            u2 = self.tau2.sigmoid() * u2 * (1 - spikefunc2(u2)) + x[t * bs:(t + 1) * bs, ...]
            u3 = self.tau3.sigmoid() * u3 * (1 - spikefunc3(u3)) + x[t * bs:(t + 1) * bs, ...] # comment this line if you want MLF (K=2)
            u = u - self.vth1.tanh()
            u2 = u2 - self.vth2.tanh()
            u3 = u3 - self.vth3.tanh()
